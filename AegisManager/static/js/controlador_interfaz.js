            ...
            tarjeta.innerHTML = `
                ...
                <div class="p-5">
                    ...
                    <div class="mt-4 flex gap-2">
                        <button class="w-full py-1.5 rounded text-[10px] font-extrabold border border-black dark:border-white bg-black dark:bg-white text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 transition-all uppercase tracking-widest">DETALLES</button>
                    </div>
                </div>
            `;
...
async function iniciarSondeoEstadisticas(nodo, id_vm, tipo) {
    const sondear = async () => {
        try {
            ...
            let estado_es = datos.status;
            if (datos.status === 'running') estado_es = 'en ejecución';
            else if (datos.status === 'stopped') estado_es = 'detenida';

            insignia.textContent = estado_es.toUpperCase();
