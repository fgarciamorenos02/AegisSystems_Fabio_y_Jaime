...
DEFAULT_HOST  = os.environ.get('PROXMOX_HOST',  '10.0.0.3')
DEFAULT_PORT  = int(os.environ.get('PROXMOX_PORT', 8006))
DEFAULT_REALM = os.environ.get('PROXMOX_REALM', 'pve')
...
def get_proxmox_connection():
    ...
    try:
        proxmox = ProxmoxAPI(
            ...
            verify_ssl=False,
            port=auth.get('port', DEFAULT_PORT),
            timeout=5
        )
        return proxmox
    except:
...
def api_login():
    data = request.json
    # Si el frontend no envía host, se usa el valor del .env
    host  = data.get('host')  or DEFAULT_HOST
    username = data.get('username')
    password = data.get('password')
    # Si el frontend no envía realm, se usa el valor del .env (pve)
    realm = data.get('realm') or DEFAULT_REALM
    ...
    try:
        ...
        proxmox = ProxmoxAPI(host, user=full_user, password=password, verify_ssl=False, port=DEFAULT_PORT, timeout=5)
